...
## Structure
- `index.html` — Home (hero + work grid, 4 projects, 1344px content width)
...
- **Play projects:** 2-column flex masonry via `.play-column` children of `.play-grid`. Each project wrapped in `.play-card` (var(--surface) bg, 1px border, 20px padding, 8px outer radius). Image on top via CSS order, title/description below with 20px margin-top on `.play-info`. Inner image radius 4px (concentric with 8px card). Click images to zoom via lightbox. Whisper XR uses `.play-card--desktop-only` (hidden ≤768px). Carousel arrows sit below image in the dots row (bottom: 24px within 84px min-height dots area) — symmetric 24px spacing on both sides.

...
- **About photo deck** (`.deck` / `.deck-track` / `.deck-card` in `about.html` + inlined IIFE, added 2026-05-29): infinite editorial drag-to-rotate carousel of the 8 personal photos. Markup triples the 8 cards (24 total) so a full set always bleeds off both edges. Built from a reference (pearl-social-nine.vercel.app) but **adapted**: the reference hijacks the mouse-wheel because its carousel IS the whole viewport (`h-screen`, page non-scrollable); our deck sits mid-page, so wheel-hijack would trap scrolling. **We use drag-to-rotate instead** — a direction-intent guard commits to a horizontal drag (grab + momentum flick, friction 0.92) but releases vertical gestures back to the page (`touch-action: pan-y`), so scrolling is never trapped. Mechanics: continuous drift `SPEED = 48` px/sec via rAF with a sub-pixel accumulator (`acc += SPEED*dt`; `Math.floor` into `scrollLeft` so it never stutters); seeded-middle on first paint (`offsetLeft + offsetWidth/2 - clientWidth/2`, in the load handler so no flash); symmetric wrap at `>=2*setWidth` / `<setWidth`. Per-frame magnetic focus (`focus = max(0, 1 - dist/halfSpan)`, `halfSpan = trackWidth*0.28`) written as `--focus` and driving `scale(0.82 + 0.22*focus)`, `opacity(0.62 + 0.38*focus)`, **squared**-falloff drop-shadow + warm-coral halo (`rgba(255,150,120,…)`, matches the footer-glow palette — deliberately NOT the reference's gold). Cards face the viewer **flat** — the reference's uniform `rotateY(-36deg)` deck-lean was tried and removed per user ("the image need to face the user"); only the cursor-driven ±8° lerped tilt + pointer-tracking glare rotate them now. Bottom-fade `mask-image` dissolves the deck into the page (use `filter: drop-shadow`, not `box-shadow`, so it renders through the mask). **Mobile (≤768px): the JS bails immediately** (`matchMedia('(max-width: 768px)')` → add `is-ready`, `return`) and CSS collapses the same markup to the original static 4×2 grid (`.deck-card:nth-child(n+9){display:none}` hides the 2 duplicate sets) — no drift/drag/tilt/halo, since none suit a phone. Respects `prefers-reduced-motion` (static scrollable row). This is a deliberate exception to the "no motion on About" preference (which still holds for Skills/Tools chips) — the deck IS the focal surface here, not incidental motion under scan content.

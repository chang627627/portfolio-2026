...
- Countersign (2026): agentic credit-analysis prototype. The name is the thesis: the agent analyzes, a human countersigns. A watchable agent loop (plan, tool calls, covenant checks, audit trail) that extracts deal financials, scores risk, and suspends itself at a human approval gate on the consequential call. The human approves the plan before anything runs, and at the gate can countersign, amend the terms, send the memo back for rework (the agent then searches for the smallest structure change that passes the same one-line decision rule), or reject with a note that goes on the record. An opt-in autonomy policy can auto-countersign only spotless first-pass approvals, after a visible countdown that any engagement holds. Front-end only with a mocked, deterministic backend. Full case study on the site at chang-mou.com/countersign. Live demo at credit-analysis-agent.vercel.app, code at github.com/chang627627/credit-analysis-agent.

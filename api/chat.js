...
- New Craft Society (2026, newest, in active development): a home for design process, not just finished pieces. Drop in rough work as it happens and the case study writes itself. For designers who still believe the work matters. Live at newcraftsociety.com. I am both a designer on it and one of the product engineers building it. It is shown as a card on the Play page; it has no case study page, so do not direct visitors to one.
- Countersign (2026): agentic credit-analysis prototype. The name is the thesis: the agent analyzes, a human countersigns. A watchable agent loop (plan, tool calls, covenant checks, audit trail) that extracts deal financials, scores risk, and suspends itself at a human approval gate on the consequential call. The human approves the plan before anything runs, and at the gate can countersign, amend the terms, send the memo back for rework (the agent then searches for the smallest structure change that passes the same one-line decision rule), or reject with a note that goes on the record. The note is never pre-filled, but it can be drafted on request from the flags already on record, and the decision carries that provenance (written by the reviewer, drafted and edited, or drafted and accepted as written) into the memo, the audit trail, and the JSON export. A monitoring agent sweeps the book on a cadence and raises covenant escalations that wait for a human. An opt-in autonomy policy can auto-countersign only spotless first-pass approvals, after a visible countdown that any engagement holds. Front-end only with a mocked, deterministic backend. Full case study on the site at chang-mou.com/countersign. Live demo at credit-analysis-agent.vercel.app, code at github.com/chang627627/credit-analysis-agent.
